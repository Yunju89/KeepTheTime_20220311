...
//                Date 자료형으로 파싱 => String 을 yyyy-MM-dd HH:mm:ss 으로 파싱해서 저장해야함.(고정된 양식으로 내려줌)
                val gson = GsonBuilder()
                    .setDateFormat("yyyy-MM-dd HH:mm:ss")   // 서버가 이런 양식으로 보내주는 String
                    .registerTypeAdapter(
                        Date::class.java,
                        DateDeserializer()
                ...
                retrofit = Retrofit.Builder()
                    ...
                    .addConverterFactory(GsonConverterFactory.create( gson ))   // gson 라이브러리와 결합 + Date 파싱 요령 첨부(gson)
